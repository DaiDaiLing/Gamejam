...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Pull()
    {
        ...
        if (tentacle.joint && Input.GetMouseButtonUp(0))
        {
            if (countHolding < 0.3f)
            {
                Debug.Log(countHolding);
                countHolding = 0f;
                rb.AddForce(tentacle.grapplePoint * 2f, ForceMode.Impulse);
        ...
        if (Input.GetMouseButtonDown(0) && !clickOneTime && tentacle.joint) 
        {
            clickOneTime = true;
        }
        else if (Input.GetMouseButtonDown(0) && clickOneTime && countHolding <= 0.35f && tentacle.joint) 
        {
            clickOneTime = false;
